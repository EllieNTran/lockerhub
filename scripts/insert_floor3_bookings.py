"""
Script to insert bookings for all lockers on floor 3.
Books all 3rd floor lockers from April 1-3, 2026 using random unique users.
"""
...
async def create_floor3_bookings():
    """Create bookings for all lockers on floor 3."""
    ...
    try:
        # Get all lockers on floor 3
        print("\nFetching lockers on floor 3...")
        lockers = await conn.fetch(GET_FLOOR3_LOCKERS_QUERY)
        print(f"Found {len(lockers)} lockers on floor 3")
        
        if not lockers:
            print("No lockers found on floor 3. Exiting.")
            return
        ...
        for i, locker in enumerate(lockers):
            ...
            try:
                ...
                print(f"✓ Locker {locker['locker_number']:>3} → {user['first_name']} ({user['email']}) - Booking ID: {booking_id}")
                success_count += 1
                
            except asyncpg.exceptions.ExclusionViolationError:
                print(f"✗ Locker {locker['locker_number']:>3} → CONFLICT (already booked)")
                error_count += 1
            except Exception as e:
                print(f"✗ Locker {locker['locker_number']:>3} → ERROR: {e}")
                error_count += 1
...
if __name__ == "__main__":
    print("=" * 60)
    print("Floor 3 Booking Creation Script")
    print("=" * 60)
