...
BEGIN
    IF EXISTS (
        ...
        AND column_name = 'booking_start'
    ) THEN
        ALTER TABLE lockerhub.bookings RENAME COLUMN booking_start TO start_date;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'lockerhub' 
        AND table_name = 'bookings' 
        AND column_name = 'booking_end'
    ) THEN
        ALTER TABLE lockerhub.bookings RENAME COLUMN booking_end TO end_date;
    END IF;
END $$;

-- Make end_date optional for requests to support permanent allocations
ALTER TABLE lockerhub.requests 
DROP CONSTRAINT IF EXISTS chk_requests_dates;
...
DO $$ BEGIN
    ALTER TABLE lockerhub.requests 
    ADD CONSTRAINT chk_requests_dates CHECK (end_date IS NULL OR end_date >= start_date);
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- Make bookings.end_date optional to support permanent allocations
ALTER TABLE lockerhub.bookings DROP CONSTRAINT IF EXISTS no_overlapping_bookings;

DO $$ BEGIN
    ALTER TABLE lockerhub.bookings ALTER COLUMN end_date DROP NOT NULL;
EXCEPTION
    WHEN others THEN null;
END $$;

ALTER TABLE lockerhub.bookings DROP CONSTRAINT IF EXISTS chk_bookings_dates;

DO $$ BEGIN
    ALTER TABLE lockerhub.bookings ADD CONSTRAINT chk_bookings_dates 
...
DO $$ BEGIN
    ALTER TABLE lockerhub.bookings ADD CONSTRAINT no_overlapping_bookings 
        EXCLUDE USING gist (
            locker_id WITH =,
