"""Process floor queue and auto-allocate lockers to waitlisted users."""
...
async def _send_waitlist_removed_notification(
    user_id: str, email: str, name: str, floor_number: str, start_date, end_date
) -> None:
    """Send notification that user was removed from waitlist."""
...
async def _send_booking_confirmation_notification(
    ...
    """Send notification that booking was created from waitlist."""
    await NotificationsServiceClient().post(
...
async def process_single_floor(floor_id: str, floor_number: str) -> int:
    ...
    Args:
        ...
        floor_number: Floor number for logging and notifications

    Returns:
        Number of allocations made for this floor
    """
    allocations_made = 0
    ...
    for request in queued_requests:
        ...
        allocations_made += 1
        logger.info("Auto-allocated locker to user from queue")

    return allocations_made


async def _get_floors_to_process(
    floor_id: Optional[str],
) -> list[dict]:
    """
    Get list of floors to process based on floor_id parameter.

    Args:
        floor_id: Optional specific floor ID, or None for all floors

    Returns:
        List of floor dictionaries with floor_id and floor_number
    """
    if floor_id:
...
async def process_floor_queue(
    floor_id: Optional[str] = None,
) -> ProcessFloorQueuesResponse:
    """
    Process floor queue(s) and auto-allocate available lockers to waitlisted users.

    When floor_id is provided (event-driven):
    - Processes queue for that specific floor only

    When floor_id is None (scheduled/manual):
    - Processes all floors with queued requests
    ...
    """
    try:
        mode = "event-driven" if floor_id else "scheduled/manual"
        logger.info(f"Processing floor queue(s) in {mode} mode")

        floors = await _get_floors_to_process(floor_id)
        ...
        if not floors:
            return ProcessFloorQueuesResponse(
                ...
                message=(
                    ...
                    if floor_id
                    else "No queued requests to process"
                ),
            )

        allocations_made = 0
        for floor in floors:
            floor_allocations = await process_single_floor(
                floor["floor_id"], floor["floor_number"]
            )
            allocations_made += floor_allocations

        logger.info(
            f"Processed {len(floors)} floor(s), made {allocations_made} allocation(s)"
        )

        return ProcessFloorQueuesResponse(
            success=True,
            allocations_made=allocations_made,
            message=f"Successfully processed queue(s). {allocations_made} locker(s) allocated.",
        )

    except Exception:
        logger.error("Error in process_floor_queue", exc_info=True)
        raise
